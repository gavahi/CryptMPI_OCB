...
#undef FUNCNAME
#define FUNCNAME MPI_OCB_Naive_Sec_Bcast
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPI_OCB_Naive_Sec_Bcast(void *buffer, int count, MPI_Datatype datatype, int root,
                         MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
#if BCAST_PRINT_FUN
   if (PRINT_FUN_NAME ){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
        printf("[Bcast rank = %d host = %s count = %d  OCB_TAG_NONCE=%d  Sec-AP = %d] Func: MPI_OCB_Naive_Sec_Bcast\n", comm_ptr->rank,hostname,count,OCB_TAG_NONCE,security_approach);fflush(stdout);
	}
#endif 	
    ...
    unsigned long   max_out_len = (unsigned long)(16 + (sendtype_sz*count));
    unsigned long decrypted_len=0;
    unsigned long plaintext_len = sendtype_sz*count;
    ...
    if (rank == root) {        
        ...
        ocb_encrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, buffer, plaintext_len, ocb_ciphertext_sendbuf, 1);
        
        init_phase=1;
        mpi_errno = MPIR_Bcast_impl(ocb_ciphertext_sendbuf, ((sendtype_sz*count)+OCB_TAG_NONCE), MPI_CHAR, root, comm_ptr, errflag);
        init_phase=0;
		...
		init_phase=1;
        mpi_errno = MPIR_Bcast_impl(ocb_ciphertext_recvbuf, ((sendtype_sz*count)+OCB_TAG_NONCE), MPI_CHAR, root, comm_ptr, errflag);
        init_phase=0;
        
        int res = ocb_decrypt(ocb_dec_ctx, ocb_dec_ctx2, ocb_dec_ctx3, ocb_dec_ctx4, ocb_ciphertext_recvbuf, plaintext_len, buffer, 1);  
