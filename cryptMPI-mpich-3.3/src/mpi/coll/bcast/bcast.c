                          ...
                          int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		...
		// printf("[Bcast [%d] [%d]  host = %s count = %d] Func: MPIR_Bcast_intra_auto\n", comm_ptr->rank,init_rank,hostname,count);fflush(stdout);
        printf("[Bcast rank = %d host = %s count = %d] Func: MPIR_Bcast_intra_auto\n", comm_ptr->rank,hostname,count);fflush(stdout);
	}
#endif 	
                    ...
                    MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		// printf("[Bcast [%d] [%d]  host = %s count = %d] Func: MPIR_Bcast_impl\n", comm_ptr->rank,init_rank,hostname,count);fflush(stdout);
        printf("[Bcast rank = %d host = %s count = %d  Intra-comm = %d] Func: MPIR_Bcast_impl\n", comm_ptr->rank,hostname,count,comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM);fflush(stdout);
	}
#endif 	
    int mpi_errno = MPI_SUCCESS;

    if(init_phase==0 && security_approach==1){
        mpi_errno = MPI_Naive_Sec_Bcast(buffer, count, datatype, root, comm_ptr, errflag);
    } else if(init_phase==0 && security_approach==2){
        mpi_errno = MPI_OCB_Naive_Sec_Bcast(buffer, count, datatype, root, comm_ptr, errflag);
    } else if(concurrent_bcast == 1 && comm_ptr->is_uniform==1 && init_phase==0 && comm_ptr->node_comm != NULL){
        mpi_errno = MPIR_Bcast_ML_Shmem_MV2(buffer, count, datatype, root, comm_ptr, errflag);
    }
...
int MPI_Bcast(void *buffer, int count, MPI_Datatype datatype, int root, MPI_Comm comm)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		// printf("[Bcast [%d] [%d]  host = %s count = %d] Func: MPI_Bcast\n", comm_ptr->rank,init_rank,hostname,count);fflush(stdout);
        printf("[Bcast rank = %d host = %s count = %d] Func: MPI_Bcast\n", comm_ptr->rank,hostname,count);fflush(stdout);
	}
#endif 	
