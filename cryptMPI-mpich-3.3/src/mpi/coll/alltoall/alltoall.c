                       ...
                       MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME){
		...
		printf("[AlltoAll rank = %d host = %s count = %d  Alltoall_tuning=%d  security_approach=%d  is_block=%d  is_block_bkup=%d] Func: MPIR_Alltoall_impl\n", comm_ptr->rank,hostname,recvcount,inter_alltoall_tuning,security_approach,comm_ptr->is_blocked,is_blocked_backup);fflush(stdout);
	}
#endif
    int mpi_errno = MPI_SUCCESS;
    ...
    comm_ptr->is_blocked = is_blocked_backup;
    
    if (security_approach == 1002 && init_phase==0)  {            
                mpi_errno = 
                    MPIR_Naive_Sec_Alltoall(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr, errflag); // Naive GCM

    } else if (security_approach == 1003 && init_phase==0)  {            
                mpi_errno = 
                    MPIR_OCB_Naive_Sec_Alltoall(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr, errflag); 

    } else if (security_approach == 1004 && init_phase==0)  {            
                mpi_errno = 
                    MPIR_OCB_unrol2_Naive_Sec_Alltoall(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr, errflag); 

    } else if (security_approach == 1005 && init_phase==0)  {            
                mpi_errno = 
                    MPIR_OCB_unrol4_Naive_Sec_Alltoall(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr, errflag); 
