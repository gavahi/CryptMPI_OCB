                               ...
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME){
		...
		printf("[AlltoAll rank=%d  host=%s  count=%d  security_approach=%d  sendtype_sz=%d] Func: MPIR_Naive_Sec_Alltoall\n", comm_ptr->rank,hostname,recvcount,security_approach,sendtype_sz);fflush(stdout);
	}
#endif 
    ...
    for( i = 0; i < comm_ptr->local_size; i++){
                        ...
                        NULL, 0))
        {
            ...
            if (PRINT_FUN_NAME){
                printf("Pass encryption Rank=%d  i=%d: Naive Alltoall\n",comm_ptr->rank,i);
                fflush(stdout);
            }
        }
#endif
    ...
    for( i = 0; i < comm_ptr->local_size; i++){
        ...
        if(!EVP_AEAD_CTX_open(ctx, ((recvbuf+dest)),
                ...
                if (PRINT_FUN_NAME){
                    printf("--- Pass Decryption Rank=%d  i=%d: Naive Alltoall\n",comm_ptr->rank,i);
                    fflush(stdout);
                }
            }                             
#endif       
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_Naive_Sec_Alltoall
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_Naive_Sec_Alltoall(const void *sendbuf,
                               int sendcount,
                               MPI_Datatype sendtype,
                               void *recvbuf,
                               int recvcount,
                               ...
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[AlltoAll rank=%d  host=%s  count=%d  OCB_TAG_NONCE=%d security_approach=%d  sendtype_sz=%d] Func: MPIR_OCB_Naive_Sec_Alltoall\n", comm_ptr->rank,hostname,recvcount,OCB_TAG_NONCE,security_approach,sendtype_sz);fflush(stdout);
	}
#endif  

    int rank, wd_size;
    rank = comm_ptr->rank;
	wd_size = comm_ptr->local_size;
    unsigned long count=0;
    unsigned long next, dest, src;
    unsigned int i;
	...
	int rem = 0;
	int plaintext_len = sendtype_sz*sendcount;
    unsigned char *ocb_ciphertext_sendbuf =large_send_buffer;
    unsigned char *ocb_ciphertext_recvbuf =large_recv_buffer;
	
	if (wd_size * plaintext_len > gcm_buf_size*1048604){ // 16 * 262144 * ocb_buf_size){
        if (rank==0) fprintf(stderr,"Array size restriction\n");
        return (mpi_errno);
    }
    ...
    for( i = 0; i < comm_ptr->local_size; i++){
        next =(unsigned long )(i*((plaintext_len) + OCB_TAG_NONCE)); 
        src =(unsigned long )(i*(plaintext_len));
		
        ocb_encrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, sendbuf+src, plaintext_len, ocb_ciphertext_sendbuf+next, 1);

    }//end for//end for

    //fprintf(stderr,"[%d] OCB Enc done",rank);

    init_phase=1;

    mpi_errno = MPIR_Alltoall_impl(ocb_ciphertext_sendbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                   ocb_ciphertext_recvbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                    comm_ptr, errflag);
    init_phase=0;

    // fprintf(stderr,"[%d] OCB Dec start",rank);
    
    for( i = 0; i < comm_ptr->local_size; i++){
        next =(unsigned long )(i*(plaintext_len+ OCB_TAG_NONCE));          
        dest =(unsigned long )(i*(plaintext_len));
        

        int res = ocb_decrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, ocb_ciphertext_recvbuf+next, plaintext_len, recvbuf+dest, 1);  
        
        if (res == -1)  printf("Authentication error\n");
       
    }

    // fprintf(stderr,"[%d] OCB Dec done",rank);
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol2_Naive_Sec_Alltoall
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol2_Naive_Sec_Alltoall(const void *sendbuf,
                               int sendcount,
                               MPI_Datatype sendtype,
                               void *recvbuf,
                               int recvcount,
                               ...
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[AlltoAll rank=%d  host=%s  count=%d  security_approach=%d] Func: MPIR_OCB_unrol2_Naive_Sec_Alltoall\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif      
	...
	wd_size = comm_ptr->local_size;
    unsigned long count=0;
    unsigned long next, dest, src, next2, dest2, src2;
    unsigned int i;
	...
	if (wd_size * plaintext_len > gcm_buf_size*1048604){ 
        if (rank==0) fprintf(stderr,"Array size restriction\n");
        return (mpi_errno);
    }

    for( i = 0; i < comm_ptr->local_size;  i+=2){
        next =(unsigned long )(i*((plaintext_len) + OCB_TAG_NONCE)); 
        src =(unsigned long )(i*(plaintext_len));

        ocb_encrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, sendbuf+src, plaintext_len, ocb_ciphertext_sendbuf+next, 2);
    ...
    for( i = 0; i < comm_ptr->local_size;  i+=2){
        ...
        res = ocb_decrypt(ocb_dec_ctx, ocb_dec_ctx2, ocb_dec_ctx3, ocb_dec_ctx4, ocb_ciphertext_recvbuf+next, plaintext_len, recvbuf+dest, 2);  
        
		if (res == -1)  printf("Authentication error 2roll\n");  //return; 
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol4_Naive_Sec_Alltoall
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol4_Naive_Sec_Alltoall(const void *sendbuf,
                               int sendcount,
                               MPI_Datatype sendtype,
                               void *recvbuf,
                               int recvcount,
                               ...
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[AlltoAll rank=%d  host=%s  count=%d  security_approach=%d] Func: MPIR_OCB_unrol4_Naive_Sec_Alltoall\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif      
...
int rank, wd_size;
	...
	wd_size = comm_ptr->local_size;
    unsigned long count=0;
    unsigned long next, dest, src, next2, dest2, src2, next3, dest3, src3, next4, dest4, src4;
    unsigned int i;
    ...
    for( i = 0; i < comm_ptr->local_size;  i+=4){
        next =(unsigned long )(i*(plaintext_len+OCB_TAG_NONCE)); // + 16+12
        src =(unsigned long )(i*(plaintext_len));
        ocb_encrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, sendbuf+src, plaintext_len, ocb_ciphertext_sendbuf+next, 4);
    ...
    for( i = 0; i < comm_ptr->local_size;  i+=4){
        ...
        res = ocb_decrypt(ocb_dec_ctx, ocb_dec_ctx2, ocb_dec_ctx3, ocb_dec_ctx4, ocb_ciphertext_recvbuf+next, plaintext_len, recvbuf+dest, 4);  
        	
			
		if (res == -1)  printf("Authentication error 4-unroll\n");  //return; 
...
if (comm_ptr->is_blocked != 1 || comm_ptr->is_uniform != 1){
#if ALLTOALL_PRINT_FUN
   if (PRINT_FUN_NAME ){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
        printf("[AlltoAll rank=%d  host=%s  count=%d  security_approach=%d  is_blocked=%d] Func: MPIR_Alltoall_Conc_ShMem_MV2 redirects to...\n", comm_ptr->rank,hostname,recvcount,security_approach,comm_ptr->is_blocked);fflush(stdout);
	}
#endif 
        int ret = MPIR_Alltoall_bruck_MV2(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr, errflag);
        return ret;
    }
   ...
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[AlltoAll rank=%d  host=%s  count=%d  security_approach=%d  is_blocked=%d] Func: MPIR_Alltoall_Conc_ShMem_MV2\n", comm_ptr->rank,hostname,recvcount,security_approach,comm_ptr->is_blocked);fflush(stdout);
	}
#endif      
    int          comm_size, i, j;
    ...
    if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] CHS 05 c=%d is_blocked=%d\n",init_rank,recvcount,comm_ptr->is_blocked);  
    ...
    if(security_approach == 2001){
        ...
        for(i=0; i<n; ++i){
            ...
            for(j=0; j<local_size; ++j){
                ...
                // Mohsen
                // non-blocked: out = ((char*)recvbuf + comm_ptr->comm_rank_list[(i*local_size + j)]*(recvcount*recvtype_extent));

                out = ((char*)recvbuf + (i*local_size + j)*(recvcount*recvtype_extent));
        ...
        // Concurrent alltoall
        if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] CHS 06 c=%d  is_blocked=%d\n",init_rank,recvcount,comm_ptr->is_blocked);  
        if(comm_ptr->is_blocked!=1){
            in = tmp_buf;
        }else{
            in = sendbuf;
        ...
        if (security_approach == 2002){
            ...
            mpi_errno = MPIR_Naive_Sec_Alltoall(in, local_size * sendcount, sendtype,
            ...
            init_phase=1;
            if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] CHS 09 c=%d\n",init_rank,recvcount);

            // MPIR_Alltoall_impl
            // MPIR_Alltoall_intra_brucks
            mpi_errno = MPIR_Alltoall_impl(in, local_size * sendcount, sendtype,
                                        shmem_buffer+local_rank*(comm_size * recvcount*recvtype_extent), local_size * recvcount, recvtype, conc_commptr, errflag);
            if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] CHS 10 c=%d\n",init_rank,recvcount);
            init_phase=0;
	...
	mpi_errno = MPIR_Barrier_impl(comm_ptr->node_comm, errflag);
	 if (mpi_errno) {
		...
		/*if(local_rank == 1 && recvcount*recvtype_extent == 8){
            ...
            //if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] CHS 08 c=%d\n",init_rank,recvcount);
                in = ((char*)shmem_buffer + idx  + j * (comm_size*recvcount*recvtype_extent));
            if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] CHS 08-1 c=%d\n",init_rank,recvcount);
    ...
    if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] CHS 11 c=%d\n",init_rank,recvcount);
