                               ...
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		...
		printf("[Gather rank = %d host = %s count = %d SA=%d] Func: MPIR_Naive_Sec_Gather\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif 
    ...
    mpi_errno = MPIR_Gather_intra_binomial(ciphertext_sendbuf, plaintext_len + 28, MPI_BYTE, ciphertext_recvbuf, plaintext_len + 28, MPI_BYTE, root, comm_ptr, &errflag);
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_Naive_Sec_Gather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_Naive_Sec_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                               void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
#if GATHER_PRINT_FUN
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Gather rank = %d host = %s count = %d SA=%d] Func: MPIR_OCB_Naive_Sec_Gather\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif 
    ...
    ocb_encrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, sendbuf, plaintext_len, ocb_ciphertext_sendbuf, 1);
    
    mpi_errno = MPIR_Gather_intra_binomial(ocb_ciphertext_sendbuf, plaintext_len + OCB_TAG_NONCE, MPI_BYTE, ocb_ciphertext_recvbuf, plaintext_len + OCB_TAG_NONCE, MPI_BYTE, root, comm_ptr, &errflag);
    ...
    if (rank == root)
    {
        ...
        for (i = 0; i < comm_ptr->local_size; i++)
        {
            next = i * (recvcount * recvtype_sz + OCB_TAG_NONCE);
            dest = i * (recvcount * recvtype_sz);
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol2_Naive_Sec_Gather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol2_Naive_Sec_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                               void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
#if GATHER_PRINT_FUN
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Gather rank = %d host = %s count = %d SA=%d] Func: MPIR_OCB_unrol2_Naive_Sec_Gather\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif 
    ...
    if (rank == root)
    {
        ...
        for (i = 0; i < comm_ptr->local_size; i+=2)
        {
            ...
            int res = ocb_decrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, ocb_ciphertext_recvbuf+next, plaintext_len, recvbuf+dest, 2);  
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol4_Naive_Sec_Gather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol4_Naive_Sec_Gather(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                               void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
#if GATHER_PRINT_FUN
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Gather rank = %d host = %s count = %d SA=%d] Func: MPIR_OCB_unrol4_Naive_Sec_Gather\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif 
    ...
    if (rank == root)
    {
        ...
        for (i = 0; i < comm_ptr->local_size; i+=4)
        {
            ...
            int res = ocb_decrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, ocb_ciphertext_recvbuf+next, plaintext_len, recvbuf+dest, 4);  
                               ...
                               MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
    ...
    //if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] gather_bio 01\n",init_rank);
    ...
    //if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] gather_bio 02\n",init_rank);
    ...
    //if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] gather_bio 03\n",init_rank);
    ...
    //if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] gather_bio 04\n",init_rank);
    curr_cnt = nbytes;
    ...
    while (mask < comm_size) {
        if ((mask & relative_rank) == 0) {
            ...
            if (!tmp_buf_size) {
                if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] gather_bio 05\n",init_rank);
                /* leaf nodes send directly from sendbuf */
                mpi_errno = MPIC_Send(sendbuf, sendcount, sendtype, dst,
                                      MPIR_GATHER_TAG, comm_ptr, errflag);
                if (mpi_errno) {
    ...
    //if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] gather_bio 06\n",init_rank);
    ...
    //if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] gather_bio 07\n",init_rank);
