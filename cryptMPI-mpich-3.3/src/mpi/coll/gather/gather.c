                     ...
                     int root, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		...
		printf("[Gather rank = %d host = %s count = %d SA=%d] Func: MPIR_Gather_impl\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif     
    int mpi_errno = MPI_SUCCESS;

    
    if (security_approach == 301 && init_phase==0)  {            
                mpi_errno = 
                    MPIR_Naive_Sec_Gather(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);

    } else if (security_approach == 311 && init_phase==0)  {            
                mpi_errno = 
                    MPIR_OCB_Naive_Sec_Gather(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);

    } else if (security_approach == 312 && init_phase==0)  {            
                mpi_errno = 
                    MPIR_OCB_unrol2_Naive_Sec_Gather(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);

    } else if (security_approach == 313 && init_phase==0)  {             
                mpi_errno = 
                    MPIR_OCB_unrol4_Naive_Sec_Gather(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);

    } else if (inter_gather_tuning>0 && security_approach>0  && init_phase==0){
        switch (inter_gather_tuning) {
            case 1:
                mpi_errno =
                    MPIR_Naive_Sec_Gather(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);
                break;
            ...
            case 3:
                mpi_errno =
                    MPIR_Gather_intra(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);
                break;
            case 4:
                mpi_errno =
                    MPIR_Gather_MV2_Direct_CHS(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);
                break;
            default:
                mpi_errno =
                    MPIR_Gather_intra(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, root, comm_ptr, errflag);
    ...
    // if (DEBUG_INIT_FILE)  fprintf(stderr,"[%d] MPIR_Gather_impl 03 c=%d \n",init_rank,recvcount);
    if (mpi_errno)
        MPIR_ERR_POP(mpi_errno);
               ...
               void *recvbuf, int recvcount, MPI_Datatype recvtype, int root, MPI_Comm comm)
{
    ...
    // if (DEBUG_INIT_FILE)  fprintf(stderr,"[%d] MPI_Gather 02 c=%d \n",init_rank,recvcount);
    if (mpi_errno)
        goto fn_fail;
  ...
  fn_exit:
    ...
    // if (DEBUG_INIT_FILE)  fprintf(stderr,"[%d] MPI_Gather 03 c=%d \n",init_rank,recvcount);
    return mpi_errno;
