...
*/
   ...
   if (PRINT_FUN_NAME){
    ...
    mpi_errno = MPIR_Scatter_intra_binomial(ciphertext_sendbuf, plaintext_len+28, MPI_CHAR,
                                            ciphertext_recvbuf, plaintext_len+28, MPI_CHAR, 
                                            root, comm_ptr, &errflag);  
    //init_phase = 0;
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_Naive_Sec_Scatter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_Naive_Sec_Scatter(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
                                MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
#if SCATTER_PRINT_FUN
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[allreduce rank=%d host=%s count=%d] Func: MPIR_OCB_Naive_Sec_Scatter\n", comm_ptr->rank,hostname,recvcount);fflush(stdout);
	}
#endif      
    ...
    if(rank == root)
    {
        ...
        for(i= 0; i< comm_ptr->local_size; i++)
        {
            ocb_encrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, sendbuf+next, plaintext_len, ocb_ciphertext_sendbuf+dest, 1);
            next += plaintext_len;
            dest += plaintext_len+OCB_TAG_NONCE;    
        }
    }

    mpi_errno = MPIR_Scatter_intra_binomial(ocb_ciphertext_sendbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                            ocb_ciphertext_recvbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR, 
                                            root, comm_ptr, &errflag);          

    int res = ocb_decrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, ocb_ciphertext_recvbuf, plaintext_len, recvbuf, 1);  
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol2_Naive_Sec_Scatter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol2_Naive_Sec_Scatter(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
                                MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
#if SCATTER_PRINT_FUN
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[allreduce rank=%d host=%s count=%d] Func: MPIR_OCB_unrol2_Naive_Sec_Scatter\n", comm_ptr->rank,hostname,recvcount);fflush(stdout);
	}
#endif      
    ...
    if(rank == root)
    {
        ...
        {
            dest =(i*(plaintext_len+ OCB_TAG_NONCE));          
            next =(i*(plaintext_len));
            ocb_encrypt(ocb_enc_ctx, ocb_enc_ctx2, ocb_enc_ctx3, ocb_enc_ctx4, sendbuf+next, plaintext_len, ocb_ciphertext_sendbuf+dest, 2);
        }
    }
    ...
    int res = ocb_decrypt(ocb_dec_ctx, ocb_dec_ctx2, ocb_dec_ctx3, ocb_dec_ctx4, ocb_ciphertext_recvbuf, plaintext_len, recvbuf, 1);  
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol4_Naive_Sec_Scatter
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol4_Naive_Sec_Scatter(const void *sendbuf, int sendcount, MPI_Datatype sendtype,
                                void *recvbuf, int recvcount, MPI_Datatype recvtype, int root,
                                MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
#if SCATTER_PRINT_FUN
   if (PRINT_FUN_NAME){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[allreduce rank=%d host=%s count=%d] Func: MPIR_OCB_unrol4_Naive_Sec_Scatter\n", comm_ptr->rank,hostname,recvcount);fflush(stdout);
	}
#endif      
    ...
    if(rank == root)
    {
        ...
        for(i= 0; i< comm_ptr->local_size; i+=4)
        {
            ...
            ocb_encrypt(ocb_dec_ctx, ocb_dec_ctx2, ocb_dec_ctx3, ocb_dec_ctx4, sendbuf+next, plaintext_len, ocb_ciphertext_sendbuf+dest, 4);

        }
    }

    mpi_errno = MPIR_Scatter_intra_binomial(ocb_ciphertext_sendbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                            ocb_ciphertext_recvbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR, root,
                                            comm_ptr, &errflag);        
