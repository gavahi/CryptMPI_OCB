                              ...
                              MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
    ...
    printf("[allreduce rank = %d host = %s count = %d] Func: MPIR_Allreduce_intra_auto\n",
    ...
    if ((nbytes <= MPIR_CVAR_ALLREDUCE_SHORT_MSG_SIZE) ||
        (HANDLE_GET_KIND(op) != HANDLE_KIND_BUILTIN) || (count < pof2)) {
            ...
            //if (PRINT_FUN_NAME){
            if (0){
               fprintf (stderr, "NOT SMP: nbytes = %d   MPIR_CVAR_ALLREDUCE_SHORT_MSG_SIZE = %d    count = %d    pof2 = %d ", nbytes, MPIR_CVAR_ALLREDUCE_SHORT_MSG_SIZE,count,pof2);
                if (nbytes <= MPIR_CVAR_ALLREDUCE_SHORT_MSG_SIZE) fprintf (stderr, " 1 = Ok");
                if (HANDLE_GET_KIND(op) != HANDLE_KIND_BUILTIN)  fprintf (stderr, " 2 = Ok");
                if (count < pof2)  fprintf (stderr, " 3 = Ok");
                fprintf (stderr, "\n");
            }
            // end
        mpi_errno =
                              ...
                              MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
    ...
    gethostname(hostname, MAX_HOSTNAME_LEN);
    printf("[allreduce rank = %d host = %s count = %d] Func: MPIR_Allreduce_inter_auto\n",
    comm_ptr->rank,hostname,count);fflush(stdout);}
#endif
    int mpi_errno = MPI_SUCCESS;
                        ...
                        MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
    ...
    gethostname(hostname, MAX_HOSTNAME_LEN);
    printf("[allreduce rank = %d host = %s count = %d SA=%d] Func: MPIR_Allreduce_impl\n",
    comm_ptr->rank,hostname,count,security_approach);fflush(stdout);}
#endif
    int mpi_errno = MPI_SUCCESS;
    
    if (init_phase==0 && inter_allreduce_tuning>0 && security_approach==2005){
        switch (inter_allreduce_tuning) {
            case 1:
                mpi_errno =
                    MPIR_Allreduce_intra_recursive_doubling_SEC(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
                break;
            case 2:
                mpi_errno =
                    MPIR_Allreduce_intra_reduce_scatter_allgather_SEC_NodeAware(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
                break;
            case 3:
                mpi_errno =
                    MPIR_Allreduce_intra_smp_SEC(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
                break;
            case 6:
                CONCUR_INTER_METHOD = 1;
                mpi_errno =
                    MPIR_Allreduce_intra_concurrent_SEC_NodeAware(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
                break;
            case 7:
                CONCUR_INTER_METHOD = 2;
                mpi_errno =
                    MPIR_Allreduce_intra_concurrent_SEC_NodeAware(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
                break;
            case 8:
                CONCUR_INTER_METHOD = 3;
                mpi_errno =
                    MPIR_Allreduce_intra_concurrent_SEC_NodeAware(sendbuf, recvbuf, count, datatype, op, comm_ptr, errflag);
                break;
            default:
    ...
    // if (DEBUG_INIT_FILE) fprintf(stderr,"[%d] End of MPIR_Allreduce_impl\n",init_rank);
    if (mpi_errno)
                   ...
                   MPI_Op op, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
	char hostname[100];
    gethostname(hostname, MAX_HOSTNAME_LEN);
    printf("[allreduce rank = %d host = %s count = %d] Func: MPIR_Allreduce\n",
    comm_ptr->rank,hostname,count);fflush(stdout);}
#endif
    int mpi_errno = MPI_SUCCESS;
                  ...
                  MPI_Datatype datatype, MPI_Op op, MPI_Comm comm)
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
    ...
    gethostname(hostname, MAX_HOSTNAME_LEN);
    printf("[allreduce rank = %d host = %s count = %d] Func: MPI_Allreduce\n",
    comm_ptr->rank,hostname,count);fflush(stdout);}
#endif
