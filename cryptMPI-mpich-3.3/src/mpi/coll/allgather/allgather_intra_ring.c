...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_Naive_Sec_Allgather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_Naive_Sec_Allgather(const void *sendbuf,
                              int sendcount,
                              MPI_Datatype sendtype,
                              void *recvbuf,
                              int recvcount,
                              MPI_Datatype recvtype, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME ){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Allgather rank = %d host = %s count = %d  Sec-AP = %d] Func: MPIR_OCB_Naive_Sec_Allgather (Naive)\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif  	
    ...
    if (comm_ptr->local_size == comm_ptr->pof2){
        mpi_errno = MPIR_Allgather_intra_recursive_doubling(ocb_ciphertext_sendbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                                            ocb_ciphertext_recvbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                                            comm_ptr, errflag);
    } else if (comm_ptr->rank == 0) {
        mpi_errno = MPIR_Allgather_intra_ring(MPI_IN_PLACE, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                            ocb_ciphertext_recvbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                            comm_ptr, errflag);
    } else {
        mpi_errno = MPIR_Allgather_intra_ring(ocb_ciphertext_sendbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                            ocb_ciphertext_recvbuf, plaintext_len+OCB_TAG_NONCE, MPI_CHAR,
                                            comm_ptr, errflag);
    }     
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol2_Naive_Sec_Allgather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol2_Naive_Sec_Allgather(const void *sendbuf,
                              int sendcount,
                              MPI_Datatype sendtype,
                              void *recvbuf,
                              int recvcount,
                              MPI_Datatype recvtype, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME ){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Allgather rank = %d host = %s count = %d  Sec-AP = %d] Func: MPIR_OCB_unrol2_Naive_Sec_Allgather (Naive)\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif  	
...
#undef FUNCNAME
#define FUNCNAME MPIR_OCB_unrol4_Naive_Sec_Allgather
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_OCB_unrol4_Naive_Sec_Allgather(const void *sendbuf,
                              int sendcount,
                              MPI_Datatype sendtype,
                              void *recvbuf,
                              int recvcount,
                              MPI_Datatype recvtype, MPIR_Comm * comm_ptr, MPIR_Errflag_t * errflag)
{
   ...
   if (PRINT_FUN_NAME ){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Allgather rank = %d host = %s count = %d  Sec-AP = %d] Func: MPIR_OCB_unrol4_Naive_Sec_Allgather (Naive)\n", comm_ptr->rank,hostname,recvcount,security_approach);fflush(stdout);
	}
#endif  	
