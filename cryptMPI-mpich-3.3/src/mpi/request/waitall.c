...
int MPI_Waitall_original(int count, MPI_Request array_of_requests[], MPI_Status array_of_statuses[])
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		...
		printf("[Waitall rank = %d host = %s SA=%d] Func: MPI_Waitall_original\n", init_rank,hostname,security_approach);fflush(stdout);
	}
#endif        
    int mpi_errno = MPI_SUCCESS;
...
int MPI_SEC_GCM_Waitall(int req_count, MPI_Request arr_req[],
                                   MPI_Status arr_sta[])
{
#if P2P_PRINT_FUN
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Waitall rank = %d host = %s SA=%d] Func: MPI_SEC_GCM_Waitall\n", init_rank,hostname,security_approach);fflush(stdout);
	}
#endif    

    int mpi_errno = MPI_SUCCESS;
    int sendtype_sz, tag, i, recv_len, len;
    unsigned long next, datasize, dest, count, totaldata, index;    
    MPI_Request request[300];
    MPI_Status sta;
    int req_no, source, rank;
    unsigned char *buf;

    // if (PRINT_FUN_NAME) printf("[%d] Waitall 000\n", init_rank);
    
    for (req_no = 0; req_no < req_count; req_no++)
    {
        index = arr_req[req_no];
        ...
        if ((index >= 0 && index < ISEND_REQ))
        {
            ...
            {                  
                ...
                {                    
                    mpi_errno = MPI_Wait_original(&arr_req[req_no], &arr_sta[req_no]);                    
                }
            }
...
int MPI_SEC_OCB_Waitall(int req_count, MPI_Request arr_req[],
                                   MPI_Status arr_sta[])
{
#if P2P_PRINT_FUN
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
		char hostname[100];
		gethostname(hostname, MAX_HOSTNAME_LEN);
		printf("[Waitall rank = %d host = %s SA=%d] Func: MPI_SEC_OCB_Waitall\n", init_rank,hostname,security_approach);fflush(stdout);
	}
#endif    
                ...
                MPI_Status array_of_statuses[])
{
   ...
   if (PRINT_FUN_NAME || DEBUG_INIT_FILE){
    ...
    if (security_approach == 401 && init_phase==0)  {            
        mpi_errno = MPI_SEC_GCM_Waitall(count, array_of_requests, array_of_statuses);
