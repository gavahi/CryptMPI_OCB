...
int MPI_Finalize(void)
{
    ...
    if(allocated_shmem==1){
        if (shmctl(shmid, IPC_RMID, 0) != 0) {
            if (init_rank==0) fprintf(stderr, "Failed to mark shm for removal\n");
        }
    }else if(allocated_shmem==2){
        
        if (shmctl(shmid, IPC_RMID, 0) != 0) {
          if (init_rank==0)  fprintf(stderr, "Failed to mark enc_buf shm for removal\n");
        }
        
        if (shmctl(ciphertext_shmid, IPC_RMID, 0) != 0) {
          if (init_rank==0)  fprintf(stderr, "Failed to mark ciphertext_shm for removal\n");
        }
    }
